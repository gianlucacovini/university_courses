...
% Consideriamo direttamente il modello completo e poi proiettiamo le soluzioni sullo
% spazio veloce-lento

% ATTENZIONE LA NULLCLINE DI V ERA SBAGLIATA
...
%% Individuazione punto P

tmin = T(V==(min(V)));

time_index = find(T==tmin);

V_min = V(time_index);
n_min = n(time_index);

%% Individuazione punto O

indices = find(T > tmin & abs(V-v0)<1e-2);
O_index = min(indices);
...
hold on
plot(T, V)  
plot(tmin, V_min, 'o', 'MarkerSize', 3, 'LineWidth', 2.5);
plot(t_O, V_O, 'o', 'MarkerSize', 3, 'LineWidth', 2.5);
grid on
% ylim([-0.5 1.2])
hold off
...
hold on
plot(V, n)
plot(V_min, n_min, 'o', 'MarkerSize', 3, 'LineWidth', 2.5);
plot(V_O, n_O, 'o', 'MarkerSize', 3, 'LineWidth', 2.5);
