function [V_vec, m_vec, h_vec, n_vec, X, T]=HH1D_solver2(f,I,IT,h,delta_t,v_x0,v_x1,V0,m0,h0,n0,sigma,C_m,alpha_m,beta_m,alpha_h,beta_h,alpha_n,beta_n)
...
%     m_vec(k,:)= m_old+delta_t*(alpha_m(V_old).*(1-m_old)-beta_m(V_old).*m_old); 
%     h_vec(k,:)= h_old+delta_t*(alpha_h(V_old).*(1-h_old)-beta_h(V_old).*h_old); 
%     n_vec(k,:)= n_old+delta_t*(alpha_n(V_old).*(1-n_old)-beta_n(V_old).*n_old); 
end  