function [V_vec, m_vec, h_vec, n_vec, X, T]=HH1D_solver_es4b(gNaSalto, I,IT,h,delta_t,v_x0,v_x1,V0,m0,h0,n0,sigma,C_m,alpha_m,beta_m,alpha_h,beta_h,alpha_n,beta_n)
...
for k=2:length(T)
    ...
    F=(C_m*I/delta_t)*V_old -G_Na(X, gNaSalto)'.*(m_old.^3).*h_old.*(V_old-V_na)-G_k.*(n_old.^4).*(V_old-V_k)-G_l.*(V_old-V_l) + Iapp_finale(X, T(k-1));
    F(1)=F(1)+v_x0/h^2;
