...
[t1, x1] = ode15s(dF, [0 140], [v0; w0], options);

v1 = x1(:, 1);
w1 = x1(:, 2);

[t2, x2] = ode15s(dF, [140 300], [v0; w0], options);
...
% è il tempo per tornare al potenziale di resting a quanto ho capito. In
% questo caso solo del secondo stimolo penso (quindi sottraiamo l'istante
% del secondo stimolo) 
% Specificare all'esame come è calcolato
[~, index_max] = max(v); 
t_in = t(index_max);
